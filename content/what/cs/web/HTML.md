...
---
## HTML
- 정의: 텍스트에 태그를 붙여 내용의 구조·의미를 기술하는 마크업 언어. 프로그래밍 언어 아님(로직·계산 없음)
- 실체: 그 자체론 문자열(inert) → 브라우저의 HTML 파서가 읽어 DOM(메모리 속 객체 트리)으로 조립 → 렌더링
  - `HTML 텍스트 → [파서] → DOM 트리 → 화면`
- XSS의 뿌리: 파서는 "믿을 마크업"과 "주입된 마크업"을 구분 못 함 → 공격자 문자열이 파서에 닿으면(`innerHTML=` 등) 그 안의 태그·속성·핸들러가 살아있는 DOM·동작이 됨
  - 입력이 '데이터' → '코드/마크업'으로 신분 변경되는 지점 = sink. 기준은 보이느냐가 아니라 해석되느냐
  - ↓ 아래 '주입 컨텍스트'·'파싱 주체'가 전부 이 한 줄 위에서 갈라짐
## 기본 구조
```
